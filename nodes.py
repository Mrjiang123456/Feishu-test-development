...
llm = ChatOpenAI(
    ...
    api_key='fd71daee-5547-477d-ba25-53db8a59fb35'
)
# 82cb3741-9d83-46fe-aeee-faad19eaf765
# fd71daee-5547-477d-ba25-53db8a59fb35
...
def create_planner_node(state: State):
    ...
    logger.info(f"{json.dumps(plan, indent=2, ensure_ascii=False)}")
...
def execute_node(state: State):
    ...
    messages = state['observations'] + [SystemMessage(content=EXECUTE_SYSTEM_PROMPT), HumanMessage(
        content=EXECUTION_PROMPT.format(user_message=state['user_message'], step=current_step['description']))]

    tool_result = None
    while True:
        response = llm.bind_tools(tools=[create_file, str_replace, shell_exec], tool_choice="auto").invoke(messages)
        response = response.model_dump_json(indent=4, exclude_none=True)
        response = json.loads(response)
        ...
        if response['tool_calls']:
            for tool_call in response["tool_calls"]:
                ...
                messages += [
                    ToolMessage(content=f"tool_name:{tool_name},tool_args:{tool_args}\ntool_result:{tool_result}",
...
def report_node(state: State):
    ...
    while True:
        response = llm.bind_tools(tools=[create_file, shell_exec], tool_choice="auto").invoke(messages)
        response = response.model_dump_json(indent=4, exclude_none=True)
        response = json.loads(response)
    ...
    return {"final_report": tool_args['file_name']}
