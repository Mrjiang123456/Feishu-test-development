...
VOLC_BEARER_TOKEN = "c67334ea-dfc7-45b0-8376-0ffb448a902b"
...
{{
  "scores": {{
    "completeness_preconditions": {{ "score": "<1-5>", "reason": "<...>" }},
    "completeness_steps": {{ "score": "<1-5>", "reason": "<...>" }},
    "completeness_expected_results": {{ "score": "<1-5>", "reason": "<...>" }},
...
def find_best_match(ai_case: Dict, golden_cases: List[Dict]) -> Union[Tuple[Dict, None], Tuple[None, str]]:
    ...
    result_candidates = call_llm(prompt_candidates, "你是一个高效的软件测试分类专家，严格按JSON输出。")
    ...
    for candidate_case in candidate_cases:
        ...
        result_score = call_llm(prompt_score, "你是一位极其严谨的软件测试架构师，严格按JSON输出。")
        time.sleep(1)
...
def main():
    ...
    for i, pair in enumerate(matched_pairs):
        ...
        eval_result = call_llm(prompt, "你是一位极其挑剔和严谨的QA总监，严格按JSON输出。")
        time.sleep(1)
        if eval_result:
